      ...
      <div className="mt-6">
        {isLoading ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <Card title="Buckets">
              <div className="text-3xl font-bold text-gold-400">{(data as any)?.total_buckets ?? '—'}</div>
            </Card>
            <Card title="Volumes">
              <div className="text-3xl font-bold text-gold-400">{(data as any)?.total_volumes ?? '—'}</div>
            </Card>
            <Card title="Storage Used">
              <div className="text-3xl font-bold text-gold-400">{(data as any)?.storage_used ?? '—'}</div>
            </Card>
          </div>
        ) : (
