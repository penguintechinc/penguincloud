...
export default function IceChartsOverview({ productId }: IceChartsProps) {
      ...
      <div className="mt-6">
        {isLoading ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <Card title="Charts">
              <div className="text-3xl font-bold text-gold-400">{(data as any)?.total_charts ?? '—'}</div>
            </Card>
            <Card title="Dashboards">
              <div className="text-3xl font-bold text-gold-400">{(data as any)?.total_dashboards ?? '—'}</div>
            </Card>
          </div>
        ) : (
