...
export default function KillKrillOverview({ productId }: KillKrillProps) {
        ...
        {isLoading ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <Card title="Resources Managed">
              <div className="text-3xl font-bold text-gold-400">{(data as any)?.total_resources ?? '—'}</div>
            </Card>
            <Card title="Active Schedules">
              <div className="text-3xl font-bold text-gold-400">{(data as any)?.active_schedules ?? '—'}</div>
            </Card>
            <Card title="Cleaned (24h)">
              <div className="text-3xl font-bold text-green-400">{(data as any)?.cleaned_24h ?? '—'}</div>
            </Card>
          </div>
        ) : (
