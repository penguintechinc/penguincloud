...
export default function Sidebar({ collapsed, onToggle }: SidebarProps) {
  ...
  const filteredNav = allNav
    .filter(
      (category) => !category.roles || hasRole(category.roles as UserRole[]),
    )
    .map((category) => ({
      ...category,
      items: category.items.filter(
        (item) => !item.roles || hasRole(item.roles as UserRole[]),
      ),
    }))
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center justify-between h-16 px-4 border-b border-slate-700">
        {!collapsed && (
          <span className="text-xl font-bold text-amber-gradient">
            PenguinCloud
          </span>
        )}
        <button
      ...
      <div className="border-t border-slate-700 p-4">
        ...
        <button
          ...
          } px-4 py-2 text-sm text-red-400 hover:bg-slate-800 rounded-lg`}
          title="Logout"
        >
          <span>🚪</span>
...
function getCategoryForType(productType: string): string {
  const map: Record<string, string> = {
    marchproxy: "infrastructure",
    squawk: "infrastructure",
    articdbm: "infrastructure",
    iceshelves: "infrastructure",
    skauswatch: "security",
    cerberus: "security",
    waddleai: "ai",
    waddlebot: "ai",
    waddleperf: "monitoring",
    icecharts: "monitoring",
    killkrill: "operations",
    tobogganing: "operations",
    darwin: "operations",
    gough: "operations",
    current: "operations",
    license_server: "operations",
    nest: "development",
    elder: "legacy",
