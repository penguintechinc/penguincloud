        ...
        {isLoading ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <Card title="Active Environments">
              <div className="text-3xl font-bold text-gold-400">{(data as any)?.active_environments ?? '—'}</div>
            </Card>
            <Card title="Templates">
              <div className="text-3xl font-bold text-gold-400">{(data as any)?.total_templates ?? '—'}</div>
            </Card>
          </div>
        ) : (
