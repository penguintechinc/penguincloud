        ...
        {isLoading ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <Card title="Active Workflows">
              <div className="text-3xl font-bold text-gold-400">{(data as any)?.active_workflows ?? '—'}</div>
            </Card>
            <Card title="Running Jobs">
              <div className="text-3xl font-bold text-gold-400">{(data as any)?.running_jobs ?? '—'}</div>
            </Card>
            <Card title="Completed (24h)">
              <div className="text-3xl font-bold text-green-400">{(data as any)?.completed_24h ?? '—'}</div>
            </Card>
          </div>
        ) : (
