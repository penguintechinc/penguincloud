...
export default function ConnectionWizard({ onComplete, onCancel }: ConnectionWizardProps) {
  ...
  return (
    <div className="card p-6 max-w-2xl mx-auto">
      ...
      {step === 1 && (
        <div>
          <p className="text-dark-400 mb-4">Select the product type to connect:</p>
          {Object.entries(categories).map(([category, types]) => (
            <div key={category} className="mb-4">
              <h3 className="text-sm font-medium text-dark-400 uppercase mb-2">{category}</h3>
                  ...
                  <button
                    ...
                    className={`text-left p-3 rounded-lg border transition-colors ${
                      selectedType?.product_type === pt.product_type
                        ? 'border-gold-400 bg-dark-800'
                        : 'border-dark-700 hover:border-dark-600'
                    }`}
                  >
                    <div className="text-sm font-medium text-gold-400">{pt.display_name}</div>
                    <div className="text-xs text-dark-500">{pt.category}</div>
                  </button>
                ))}
              </div>
            </div>
      ...
      {step === 2 && (
        <div className="space-y-4">
          ...
          <div>
            <label className="block text-sm text-dark-400 mb-1">Auth Type</label>
            <select
          ...
          {formData.auth_type !== 'none' && (
            <div>
              <label className="block text-sm text-dark-400 mb-1">API Key / Token</label>
              <input
                type="password"
          ...
          {formData.auth_type === 'basic' && (
            <div>
              <label className="block text-sm text-dark-400 mb-1">API Secret / Password</label>
              <input
                type="password"
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-sm text-dark-400 mb-1">Health Endpoint</label>
              <input
                type="text"
            ...
            <div>
              <label className="block text-sm text-dark-400 mb-1">API Version</label>
              <input
                type="text"
          ...
          <div className="flex gap-3 pt-2">
            ...
            <button onClick={handleSubmit} disabled={isSubmitting || !formData.base_url} className="btn btn-primary flex-1">
      ...
      <button onClick={onCancel} className="mt-4 text-sm text-dark-400 hover:text-dark-300">
        Cancel
      </button>
    </div>
  );
