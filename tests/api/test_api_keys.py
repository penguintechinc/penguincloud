...
class TestAPIKeyCreation:
    ...
    @pytest.mark.asyncio
    async def test_api_key_format(self, client, auth_headers):
        """Test API key format"""
        response = await client.post(
    ...
    @pytest.mark.asyncio
    async def test_create_key_with_expiration(self, client, auth_headers):
...
class TestAPIKeyListing:
    ...
    @pytest.mark.asyncio
    async def test_list_keys_no_secret(self, client, auth_headers):
...
class TestAPIKeyUsage:
    ...
    @pytest.mark.asyncio
    async def test_request_with_api_key(self, client):
    ...
    @pytest.mark.asyncio
    async def test_api_key_not_found(self, client):
    ...
    @pytest.mark.asyncio
    async def test_api_key_expired(self, client):
...
class TestAPIKeyRevocation:
    ...
    async def test_revoke_api_key(self, client, auth_headers):
        ...
        # Revoke it
        response = await client.delete(f"/api/v1/api-keys/{key_id}", headers=auth_headers)  # noqa: E501

        assert response.status_code == 204

    @pytest.mark.asyncio
    async def test_use_revoked_key(self, client, auth_headers):
...
class TestAPIKeyScopes:
    ...
    @pytest.mark.asyncio
    async def test_key_with_limited_scopes(self, client, auth_headers):
    ...
    @pytest.mark.asyncio
    async def test_key_permission_enforcement(self, client, auth_headers):
