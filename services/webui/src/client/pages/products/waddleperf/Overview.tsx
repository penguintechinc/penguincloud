        ...
        {isLoading ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <Card title="Benchmarks Run">
              <div className="text-3xl font-bold text-gold-400">{(data as any)?.total_benchmarks ?? '—'}</div>
            </Card>
            <Card title="Reports">
              <div className="text-3xl font-bold text-gold-400">{(data as any)?.total_reports ?? '—'}</div>
            </Card>
            <Card title="Avg Score">
              <div className="text-3xl font-bold text-green-400">{(data as any)?.avg_score ?? '—'}</div>
            </Card>
          </div>
        ) : (
