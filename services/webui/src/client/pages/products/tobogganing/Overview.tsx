        ...
        {isLoading ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <Card title="Active Rollouts">
              <div className="text-3xl font-bold text-gold-400">{(data as any)?.active_rollouts ?? '—'}</div>
            </Card>
            <Card title="Pipelines">
              <div className="text-3xl font-bold text-gold-400">{(data as any)?.total_pipelines ?? '—'}</div>
            </Card>
            <Card title="Deployments (24h)">
              <div className="text-3xl font-bold text-green-400">{(data as any)?.deployments_24h ?? '—'}</div>
            </Card>
          </div>
        ) : (
