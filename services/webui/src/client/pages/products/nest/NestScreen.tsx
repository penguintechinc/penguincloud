...
/**
 * Shell shared by every Nest screen: gate, then connection, then content.
 *
 ...
 *
 * This check is the RENDER gate only. The fetch gate is in `useNestConnection`,
 * and both are required: hooks run before a component decides what to render,
 * so a screen that returned this placeholder without the hook-level gate would
 * have already pulled the tenant's estate into the cache.
 *
 ...
 * believes, and `products:nest:*` scopes are only minted for tenants actually
 * connected to Nest.
 */
export function NestScreen({
  title,
  description,
  productId,
  isConnectionLoading,
  ...
  if (productId === undefined) {
    return (
      <EmptyState
        title="No Nest connection"
        description="Register a Nest connection for this tenant to manage its data resources."
        dataTestId="nest-no-connection"
      />
    );
  }

  return (
    <div data-testid="nest-screen">
